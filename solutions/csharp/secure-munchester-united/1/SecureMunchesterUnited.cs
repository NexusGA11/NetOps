...
public class SecurityPassMaker
{
    ...
    {
        // Se for exatamente Security (não subclasses), adicionar o sufixo.
        if (support.GetType() == typeof(Security))
        {
            return ((Security)support).Title + " Priority Personnel";
        }

        // Se for qualquer Staff (inclui subclasses de Security), retorna o Title.
        if (support is Staff staffMember)
        {
            return staffMember.Title;
        }

        // Caso contrário (Manager, Chairman, ...)
